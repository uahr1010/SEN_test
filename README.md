...
뉴스는 **한국어 원문**(`content/news.json`)과 **번역**(`content/news-i18n.json`)이
서로 다른 파일에 나뉘어 있습니다. Pages CMS는 저장할 때 폼에 없는 필드를
통째로 지워 버려서(병합이 아니라 덮어쓰기), `[② 뉴스]`를 한국어 전용으로
두려면 번역을 아예 다른 파일(다른 탭)에 둘 수밖에 없었습니다.
