...
| 메뉴 | 편집 내용 | 파일 |
|---|---|---|
| ② 뉴스 | 뉴스 추가·수정 (한국어만, 기사 한 건=파일 한 개) | `content/news/*.json` → `content/news.json` 자동 생성 |
...
② 뉴스에서 **새 파일 추가**(Add new) → 기사 한 건이 파일 한 개로 저장됩니다.
목록 위 "분류" 열 머리글을 누르면 분류별로 묶어 볼 수 있고, 기본적으로도
분류순으로 정렬돼 열립니다.
...
저장하면 GitHub Actions(`뉴스 합치기`)가 `content/news/` 폴더의 모든 기사를
자동으로 `content/news.json` 하나로 합쳐 사이트에 반영합니다(1분 이내).
**기사 한 건을 추가·수정해도 다른 기사 파일은 전혀 건드리지 않습니다** —
예전에는 기사들이 배열 하나에 다 들어 있어서 한 건만 추가해도 기존 기사가
전부 지워지는 사고가 있었는데, 파일을 나누면서 이 문제가 없어졌습니다.
...
뉴스는 **한국어 원문**(`content/news/*.json`, 기사 한 건당 파일 하나 →
GitHub Actions가 `content/news.json` 하나로 자동 합침)과 **번역**
